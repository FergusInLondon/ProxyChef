...
describe('[rules.js] Rules', () => {
    const mockUrls = [
        ...
        "https://fergus.london/blog",   "http://mit.edu/",
    ...
    describe('ruleForUrl', () => {
        it('should not match to an empty pattern list', () => {
            rulesEngine.load('spec/fixtures/*.nonexistant.json')
            assert.equal(mockUrls.filter((url) => rulesEngine.ruleForUrl(url) != null).length, 0);
        })

        it('should match on domain names', () => {
            rulesEngine.load('spec/fixtures/*.domain.json')
            assert.equal(mockUrls.filter((url) => rulesEngine.ruleForUrl(url) != null).length, 1);
        })
        ...
        it('should match on protocols', () => {
            ...
            assert.equal(mockUrls.filter((url) => rulesEngine.ruleForUrl(url) != null).length, 6);
        })
        ...
        it('wildcard should match all request routes', () => {
            ...
            assert.equal(mockUrls.filter((url) => rulesEngine.ruleForUrl(url) != null).length, mockUrls.length);
        })
    })
})
